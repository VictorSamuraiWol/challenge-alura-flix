...
function Cavaleiro({ borderColor, borderColorBottom, boxShadow, titulo }) {
    ...
    console.log(cavaleiros.map(cav => console.log(cav.tipo)))

    return (
        <div className={styles.grupo}>
            {cavaleiros.map(cavaleiro => {
                return cavaleiro.tipo === titulo && <div style={{ border: `3px solid ${borderColor}`, boxShadow: `0px 0px 20px ${boxShadow}` }} key={cavaleiro.nome} className={styles.elementCavaleiro}>
                    <iframe
